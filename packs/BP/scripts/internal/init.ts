/*
  ...
  If you want to add your own code, do so in the main.ts file within BP/scripts/main.ts
*/

import { Block, BlockPermutation, Dimension, Player, system, world } from '@minecraft/server';
import "../main"; // User provided main function
import luckyEventData from '../events'; // User provided lucky block events

export type LuckyBlockBreakEvent = {
    block: Block,
    dimension: Dimension,
    player: Player,
...
export type LuckyBlockEventData = {
    author: string,
    events: ((e: LuckyBlockBreakEvent) => void)[]
}

// This function handles rolling a lucky block function that is present in the luckyBlockEvents object array.
export function rollLuckyBlock(ctx: LuckyBlockBreakEvent) {
    if (luckyEventData.author == "Your username here") {
        world.sendMessage("§eWarning: please set the author field in BP/events.ts to your username")
    }
    const luckyBlockEvents = luckyEventData.events

    if (luckyBlockEvents.length == 0) {
        world.sendMessage("§cYou have not added any events to events.ts.")
    ...
    const selectedEvent = luckyBlockEvents[eventIndex] as ((e: LuckyBlockBreakEvent) => void)
...
system.beforeEvents.startup.subscribe(({ blockComponentRegistry }) => {
    blockComponentRegistry.registerCustomComponent('bao_30k:lucky_block', {
        onPlayerBreak: (e => {
            if (!e.player) return
            const ctx: LuckyBlockBreakEvent = {
                block: e.block,
